...
for K in K_set:
    ...
    #print(z_star.T)
    real_ind=np.argmax(z_star.T,axis=0)
    print(real_ind)
    for _ in range(10):
        data=simulation(100,K)
        cluster_num = K  # 聚类数
        print(data)
        ...
        distance = index_dist(cluster_num, ind_est, ind_star)
        with open(file_path, 'a') as f:
            f.write(f'{K},{distance}\n')
        distances.append(distance)
    # 计算当前 K 值下的平均距离（期望聚类误差）
    expected_distance = np.mean(distances)
    # 将 K 值和计算出的平均距离添加到结果 DataFrame 中
    new_row = pd.DataFrame({'K': [K], 'Distance': [expected_distance]})
    results_df = pd.concat([results_df, new_row], ignore_index=True)
# 使用 matplotlib 绘制结果
plt.figure(figsize=(10, 5))
plt.plot(results_df['K'], results_df['Distance'], marker='o')
plt.xlabel('K (Number of Clusters)')
plt.ylabel('Expected Clustering Error')
plt.title('Expected Clustering Error for different values of K')
plt.grid(True)
plt.savefig("simulation/expected error cluster distance,tau0.5,k=5~25.png", dpi=600)
plt.show()
